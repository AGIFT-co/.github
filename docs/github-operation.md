...
- Project 紐づけ: Issue を対象の組織 Project へ紐づける
...
各プロダクトリポジトリの Issue を、組織横断の Project へ集約する。
対象 Project は環境変数 `BOARD_OWNER`・`BOARD_PROJECT_TITLE`（Project タイトル）で指定する（「ボード操作スクリプト（正本）」参照）。
...
stateDiagram-v2
    ...
    [*] --> Backlog : board-add.sh
    Backlog --> Ready : board-ready.sh
    Ready --> InProgress : board-start.sh
    InProgress --> Review : board-finish.sh
    Review --> Done : 手動（全体 Weekly）
    Done --> [*] : board-archive-done.sh
    note right of Done
        Status=Done かつ open の Issue は
        board-close-done.sh で close 整合（冪等）
    end note
```
...
スクリプトの対象 Project は環境変数で指定する（製品固有の既定値は持たない）。

| 環境変数 | 要否 | 用途 |
|---|---|---|
| `BOARD_OWNER` | 必須 | Project の所有者（組織名またはユーザー名） |
| `BOARD_PROJECT_TITLE` | 推奨 | Project のタイトル。番号を実行時に解決するため、設定にマジックナンバー（番号）を置かずに済む |
| `BOARD_PROJECT_NUMBER` | 任意 | Project 番号。指定時は `BOARD_PROJECT_TITLE` より優先する |
| `BOARD_RAW` | 任意 | `1` を指定するとフック `guard-board-query.sh` の生コマンド拒否を回避する（例外用） |

`BOARD_OWNER` は必須、対象 Project は `BOARD_PROJECT_TITLE` か `BOARD_PROJECT_NUMBER` のいずれかで指定する。
いずれも未指定のときはスクリプトが中断する。
